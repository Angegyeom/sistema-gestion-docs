...
const MainContent = ({ activeSystem, showHome, sidebarExpanded, toggleSidebar }) => {
    if (!activeSystem) {
        return (
            <main className={`ml-72 transition-all duration-300 p-6 pt-28`}>
    ...
    return (
        <main className={`transition-all duration-300 h-screen pt-[88px] ${sidebarExpanded ? 'ml-96' : 'ml-72'}`}>
        ...
        <div className="h-full flex flex-col">
            <header className="bg-white p-4 border-b flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                    {system.credentials && <button onClick={() => toggleSidebar(!showCredentials)} className="bg-[#1976D2] hover:bg-[#0D47A1] text-white text-sm py-1.5 px-3 rounded">🔑 Credenciales</button>}
                    <button onClick={() => window.open(system.url, '_blank')} className="bg-[#1976D2] hover:bg-[#0D4T7A1] text-white text-sm py-1.5 px-3 rounded">🔗 Nueva Ventana</button>
            ...
            <div className="flex-1">
                 {system.id === 'capacitacion' ? (
                     <div className="w-full h-full bg-no-repeat bg-contain bg-center cursor-pointer" style={{backgroundImage: "url('/images/Sistemas/capacitacion.png')"}} onClick={() => window.open(system.url, '_blank')}>
                         <div className="w-full h-full bg-[#667eea]/90 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity">
